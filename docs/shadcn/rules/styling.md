...
主题、CSS 变量和添加自定义颜色见 [customization.md](../customization.md)。
...
如果需要一个不存在于语义化 token 中的 success/positive 颜色，请使用 Badge variant，或询问用户是否要向主题添加自定义 CSS 变量（见 [customization.md](../customization.md)）。
...
使用语义化 tokens，它们会通过 CSS 变量处理 light/dark。使用 `bg-background text-foreground`，不要使用 `bg-white dark:bg-gray-950`。

---

## 使用 cn() 处理条件 class

使用项目中的 `cn()` 工具处理条件或合并 class name。不要在 className 字符串中手写三元表达式。
...
`Dialog`、`Sheet`、`Drawer`、`AlertDialog`、`DropdownMenu`、`Popover`、`Tooltip`、`HoverCard` 会自行处理层叠关系。绝不要添加 `z-50` 或 `z-[999]`。
