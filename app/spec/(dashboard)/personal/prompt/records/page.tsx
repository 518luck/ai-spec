...
export default async function Page({
	...
	searchParams: Promise<{ folderId?: string; tagIds?: string; q?: string; filter?: string }>;
}) {
	// Next.js 15 的 searchParams 是 Promise，必须先 await 再校验
	const sp = await searchParams;
	// 用 Zod 校验：合法值保留，非法值校验失败直接抛错
	const { folderId, tagIds, q, filter } = listRecordsDtoSchema.parse(sp);
	return <PersonalRecordsPage folderId={folderId} tagIds={tagIds} q={q} filter={filter} />;
}
