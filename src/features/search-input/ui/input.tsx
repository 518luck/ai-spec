...
import { buildDefaultFilter } from "../config/filters";
import type { SearchInputProps } from "../model/types";
import { FilterPopover } from "./filter-popover";
import { InputField } from "./input-field";

// > 一体化胶囊：所有元素共享一个边框容器；不传 filters 时不渲染筛选按钮，退化为纯搜索框
export function Input({ filters, defaultFilter, className }: SearchInputProps): JSX.Element {
	...
	return (
		...
		>
			<InputField defaultFilters={defaultFilters} />
			{filters?.length ? (
				<>
					{/* 输入框与筛选区的分隔线 */}
					<div className="h-5 w-px shrink-0 bg-border" />
					<FilterPopover filters={filters} defaultFilters={defaultFilters} />
