...
import { getPlaceholder, SEARCH_DEBOUNCE_MS, SEARCH_QUERY_PARAM } from "../config/filters";
import { useUrl } from "../model/use-url";
...
export function InputField({ defaultFilters, className }: InputFieldProps): JSX.Element {
	// 搜索词写入的 URL 参数名固定为 q（对齐 Linear）
	const param = SEARCH_QUERY_PARAM;
	const { getParam, setParam, deleteParam, getFilters } = useUrl(defaultFilters);
