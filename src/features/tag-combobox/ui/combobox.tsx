...
// > 由外层容器（Filter、editor-toolbar 等）自行包 Popover 和 chips 展示
					...
					<CommandGroup>
						...
						>
							<span
								...
								style={{
									backgroundColor: `color-mix(in srgb, ${NEUTRAL_COLOR} 15%, transparent)`,
								}}
							>
								<Icons.tagAdd className="size-4" style={{ color: NEUTRAL_COLOR }} />
							</span>
							<span>新建标签</span>
						</CommandItem>
					</CommandGroup>
...
function CreateButtonOnEmpty({ onSelect }: { onSelect: (name: string) => void }): JSX.Element {
	...
	return (
		...
		>
			<span
				className="flex size-7 shrink-0 items-center justify-center rounded-md"
				style={{ backgroundColor: `color-mix(in srgb, ${NEUTRAL_COLOR} 15%, transparent)` }}
			>
				<Icons.plus className="size-4" style={{ color: NEUTRAL_COLOR }} />
			</span>
