...
// # 面板触发器：可选预设（过滤/布局）+ 菜单槽位 + 右侧已选条件槽位
// > 只负责视觉与开合；具体筛选项（标签/组织/收藏…）和 chips 由调用方传入
...
// > 面板触发器：根据 variant 渲染对应图标与文本，菜单与 chips 完全外置
export function PanelTrigger({
	...
	const { icon, label } = PANEL_TRIGGER_PRESETS[variant];
