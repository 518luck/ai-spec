...
import { SIDEBAR_COMPACT_WIDTH, SIDEBAR_MAX_WIDTH, SIDEBAR_MIN_WIDTH } from "../model/config";
import type {
	NavBusinessArea,
...
type SidebarProps = Omit<ComponentProps<typeof motion.aside>, "children">;
...
export function Sidebar({ className, ...props }: SidebarProps): JSX.Element {
	...
	const { width, collapsed, isResizing, toggleCollapsed } = useSidebarContext();
	...
	return (
		...
		>
			...
			<nav
				...
				className={cn(
					sidebarZoneClasses.businessNav.shell,
...
function NavAreasPanel({
	...
	const { collapsed, toggleCollapsed, resetWidth, navigate } = useSidebarContext();
	...
	return (
		<nav
			...
			data-state={collapsed ? "collapsed" : "expanded"}
			className={cn(
				sidebarZoneClasses.operationNav.shell,
		...
		>
			<div
				...
				className={cn(
					sidebarZoneClasses.operationNav.surface,
