...
type KbdProps = React.ComponentProps<"kbd"> & {
	...
	// 窄屏（<lg, 1024px）时隐藏：用于按钮文字旁的快捷键提示，窄屏省出空间；tooltip/文档等场景不应开启
	hideOnNarrow?: boolean;
};

function Kbd({ className, alignWithText = false, hideOnNarrow = false, ...props }: KbdProps) {
	return (
		<kbd
			data-slot="kbd"
			className={cn(
				...
				hideOnNarrow && "hidden lg:inline-flex",
				className,
			)}
			{...props}
		/>
