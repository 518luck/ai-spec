...
- `ohs/`：Open Host Service 接口层，向外暴露应用服务与 HTTP/REST 适配能力。
  - `ohs/local/appservice/`：应用服务层（用例编排、事务），对应 React Server Actions。
  - `ohs/remote/adapter/`：HTTP 适配器（鉴权包装、错误响应、请求上下文提取等）。
  - `ohs/remote/controller/`：HTTP 控制器层，仅保留控制器逻辑，不混入适配器工具。
  - `ohs/remote/routers/`：路由注册薄层。
  - `ohs/pl/`：ports 接口抽象层。
- `auth/`：NextAuth 配置、session 读取、OTP 和认证相关常量。
- `infrastructure/`：Redis、Axiom（日志）、Resend/React Email、BullMQ 队列、环境配置等第三方服务适配层。
...
- 新增 Server Action 放在 `ohs/local/appservice/` 下，文件顶部保留 `"use server"`，并复用 `ohs/local/appservice/safe-action.ts` 的 `actionClient` 处理输入校验和服务端错误。
...
- `ohs/remote/adapter/` 只放本目录复用的 HTTP 适配器；新增 HTTP 入口仍放在根目录 `app/api/**`。
...
- 新增或修改 RBAC 资源、权限动作时，同时检查 `ohs/local/appservice/rbac/resources.ts` 和 `ohs/local/appservice/rbac/permissions.ts`，保持资源 key、权限 action 与角色映射一致。
