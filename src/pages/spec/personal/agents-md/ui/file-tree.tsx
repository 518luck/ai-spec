		...
		<div className="flex flex-col">
			...
			>
				{tree.getItems().map((item) => (
					<FileTreeRow key={item.getId()} item={item} />
				))}
			</div>
		</div>
	);
...
function FileTreeRow({ item }: { item: ItemInstance<AgentsTreeItem> }): JSX.Element {
	const isExpanded = item.isExpanded();
	const { level } = item.getItemMeta();
	// 项目根用 root 图标，其余按文件夹名解析专属图标（未命中回退通用文件夹）
	const iconPair = level === 0 ? rootFolderIcon : resolveFolderIcon(item.getItemName());
	const iconSrc = isExpanded ? iconPair.open.src : iconPair.closed.src;
