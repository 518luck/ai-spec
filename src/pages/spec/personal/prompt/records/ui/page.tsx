...
import type { ListRecordsDto } from "@/shared/lib/zod/schemas/prompt/record";
...
export function PersonalRecordsPage({
	...
	const listParams = { folderId, tagIds, q, filter, favorite, sort };
	const { data, isLoading, isFetchingNextPage, fetchNextPage, hasNextPage } = useInfiniteQuery({
		queryKey: recordKeys.infinite(listParams),
		queryFn: ({ pageParam }) =>
			// pageParam 为 1-based 页码（initialPageParam 给 1），直接传给后端 list procedure
			client.records.list({ ...listParams, page: pageParam }),
		initialPageParam: 1,
