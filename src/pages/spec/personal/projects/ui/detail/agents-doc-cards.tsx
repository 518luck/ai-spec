...
interface AgentsDocCardGridProps {
	docs: AgentsDocEntry[];
	/** 点击卡片时上抛文档 id，右侧切换为阅读视图 */
	onOpen: (fileId: string) => void;
}

export function AgentsDocCardGrid({ docs, onOpen }: AgentsDocCardGridProps): JSX.Element {
	return (
		<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
			{docs.map((doc) => (
				<AgentsDocCard key={doc.fileId} doc={doc} onOpen={onOpen} />
			))}
		</div>
	);
}

interface AgentsDocCardProps {
	doc: AgentsDocEntry;
	onOpen: (fileId: string) => void;
}

// 单张卡片：标题按钮的伪元素铺满卡面实现整卡可点，底部标注文档所在文件夹路径
function AgentsDocCard({ doc, onOpen }: AgentsDocCardProps): JSX.Element {
	return (
