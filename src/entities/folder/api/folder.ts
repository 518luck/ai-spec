...
import type { FolderOptionVoType } from "@/shared/lib/zod/schemas/folder";

// > 获取当前用户的文件夹列表（GET /api/folders），按资源类型过滤；非 2xx 时解析后端统一错误体并抛出
export const getFolders = async (type: string): Promise<FolderOptionVoType[]> => {
	const response = await fetch(`/api/folders?type=${encodeURIComponent(type)}`);
	if (!response.ok) {
		throw new Error(await resolveErrorMessage(response));
	}
	return (await response.json()) as FolderOptionVoType[];
};
