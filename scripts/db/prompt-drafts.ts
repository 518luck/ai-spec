...
// 生成目标数量
const DRAFT_COUNT = 250;

// 草稿模板，按业务主题分组，每组生成正式、生产环境风格的段落内容
...
const generateDraft = (
	...
	const template = draftTemplates[index % draftTemplates.length];
	const serial = Math.floor(index / draftTemplates.length) + 1;
	const baseDate = new Date(2026, 6, 10, 9, 0, 0, 0);
	const createdAt = new Date(baseDate.getTime() + index * 7 * 60 * 1000);
	const updatedAt = new Date(createdAt.getTime() + Math.floor(Math.random() * 60) * 1000);
...
// 主流程：先清空当前用户的所有草稿，再批量写入固定数量的新草稿
const main = async (): Promise<void> => {
	const deleted = await prisma.promptDraft.deleteMany({
		where: { ownerId: OWNER_ID },
	});

	const drafts = Array.from({ length: DRAFT_COUNT }, (_, index) => generateDraft(index));

	const result = await prisma.promptDraft.createMany({
		data: drafts,
	});
