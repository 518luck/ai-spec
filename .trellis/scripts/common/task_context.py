...
def _validate_jsonl(jsonl_file: Path, repo_root: Path, task_dir: Path | None = None) -> int:
    ...
    for line in jsonl_file.read_text(encoding="utf-8").splitlines():
        ...
        if extension in _CODE_FILE_EXTENSIONS and not _is_exempt_from_code_file_warning(
            ...
            print(
                f"  {colored(f'{file_name}:{line_num}: Warning: {file_path} looks like a code file — implement/check.jsonl should reference spec/research docs; agents read code themselves', Colors.YELLOW)}"
            )
        ...
        if max_file_bytes:
            ...
            if size > max_file_bytes:
                print(
                    f"  {colored(f'{file_name}:{line_num}: Warning: {file_path} is {size} bytes, exceeds context_injection.max_file_bytes ({max_file_bytes}); injection will truncate it', Colors.YELLOW)}"
                )
